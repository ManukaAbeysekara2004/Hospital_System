...
export default function ReceptionistRegistration({ onBackToRoles, onBackToLogin }) {
  const [step, setStep] = useState(1); // 1: Personal Info, 2: Receptionist Details, 3: Account Setup
  const [errorMessage, setErrorMessage] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [successModal, setSuccessModal] = useState(false);
  const [registeredReceptionist, setRegisteredReceptionist] = useState(null);
  ...
  const handleFinalSubmit = async (e) => {
    ...
    if (!trimmedEmail.endsWith('@gmail.com')) {
      setErrorMessage('Email address must end with @gmail.com (e.g. reception@gmail.com).');
      return;
    }
    ...
    const formattedLanguages = languagesList.map(l => ({ Language: l }));
    ...
    const receptionistData = {
      ...
      AssignedDeskCounter: assignedDeskCounter.trim(),
    ...
    try {
      const response = await fetch('http://localhost:5000/api/receptionist/registration', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setIsLoading(false);
        setRegisteredReceptionist(receptionistData);
        setSuccessModal(true);
      } else {
        setIsLoading(false);
        setErrorMessage(resData.message || resData.error || 'Receptionist registration failed. Please check your inputs.');
      }
    } catch (err) {
  ...
  return (
    <div className="login-card animate-fade-in" style={{ gridTemplateColumns: '1fr', maxWidth: '980px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <div className="stepper-pills">
              ...
              <span className={`step-pill ${step >= 2 ? 'active' : ''}`}>2. Desk Details</span>
              <span className="step-line"></span>
          ...
          <div className="auth-header" style={{ marginBottom: '24px' }}>
            ...
            <h2 className="auth-title" style={{ marginTop: '8px' }}>
              ...
              {step === 2 && 'Receptionist & Desk Information'}
              {step === 3 && 'Account Setup'}
            </h2>
            <p className="auth-subtitle">
              {step === 1 && 'Provide your personal details & contact information as front desk receptionist.'}
              {step === 2 && 'Specify your Employee ID, assigned front desk counter, & spoken languages.'}
              {step === 3 && 'Setup your official hospital email (@gmail.com) & secure receptionist password.'}
            </p>
          </div>
          ...
          {step === 1 && (
            <form className="auth-form animate-fade-in" onSubmit={handleNextStep1}>
              <div className="input-group">
                <label className="input-label" htmlFor="recFullName">Full Name</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div className="input-group">
                  <label className="input-label" htmlFor="recGender">Gender</label>
                  <div className="input-field-wrapper">
                    <select
                      id="recGender"
                      className="input-field"
                ...
                <div className="input-group">
                  <label className="input-label" htmlFor="recNIC">NIC Number</label>
                  <div className="input-field-wrapper">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div className="input-group">
                  <label className="input-label" htmlFor="recPhone">Phone Number (Must be 10 digits)</label>
                  <div className="input-field-wrapper">
                ...
                <div className="input-group">
                  <label className="input-label" htmlFor="recAddress">Address</label>
                  <div className="input-field-wrapper">
              ...
              >
                Next to Desk Details
                <ArrowRight size={20} />
              </button>
            </form>
          )}

          {/* STAGE 2 FORM: RECEPTIONIST DETAILS */}
          {step === 2 && (
            <form className="auth-form animate-fade-in" onSubmit={handleNextStep2}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div className="input-group">
                  <label className="input-label" htmlFor="recEmpID">Employee ID</label>
                  <div className="input-field-wrapper">
              ...
              {/* Spoken Languages Tag Input */}
              <div className="input-group">
                <label className="input-label">Spoken Languages</label>
                <div className="input-field-wrapper">
                  ...
                  <input
                    ...
                    placeholder="Type language & click Add"
                    value={languageInput}
          ...
          {step === 3 && (
            <form className="auth-form animate-fade-in" onSubmit={handleFinalSubmit}>
              <div className="input-group">
                <label className="input-label" htmlFor="recEmail">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div className="input-group">
                  <label className="input-label" htmlFor="recPwd">Password</label>
                  <div className="input-field-wrapper">
                ...
                <div className="input-group">
                  <label className="input-label" htmlFor="recCpwd">Confirm Password</label>
                  <div className="input-field-wrapper">
      ...
      {/* SUCCESS MODAL AFTER RECEPTIONIST REGISTRATION */}
      {successModal && registeredReceptionist && (
        <div className="modal-overlay animate-fade-in">
          <div className="modal-content">
            ...
            <p>
              Receptionist <strong>{registeredReceptionist.FullName}</strong> has been registered at <strong>{registeredReceptionist.AssignedDeskCounter}</strong>.
            </p>
            ...
            <div style={{ background: 'var(--bg-input)', padding: '16px', borderRadius: '10px', marginBottom: '24px', fontSize: '0.88rem', textAlign: 'left', color: 'var(--text-muted)' }}>
              <div style={{ marginBottom: '4px' }}>• Employee ID: <span style={{ color: 'var(--text-main)', fontWeight: 'bold' }}>{registeredReceptionist.EmployeeID}</span></div>
              <div style={{ marginBottom: '4px' }}>• Phone: <span style={{ color: 'var(--text-main)' }}>{registeredReceptionist.PhoneNumber}</span></div>
              <div style={{ marginBottom: '4px' }}>• Email: <span style={{ color: '#10b981', fontWeight: 'bold' }}>{registeredReceptionist.Email}</span></div>
            </div>
